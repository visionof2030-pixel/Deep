...
تكتب هذا التقرير بصفتك مشرفًا تربويًا يحلل بيانات واقعية ويستند إلى مؤشرات أداء واضحة.

التقرير: "{report_name}"
التصنيف: "{subcategory_name}"
الجدارة: "{criterion_name}"
...
اعرض تحليلًا واقعيًا لما تم رصده، موضحًا المؤشرات، وأسباب النتائج، والإجراءات التي اتخذتها بناءً على البيانات.

ركّز على:
- ما الذي كشفته المؤشرات
- كيف فسّرت النتائج
...
- الإجراءات في 4 نقاط قصيرة.
- لا تكرر نفس المفردات بين الحقول.
- اذكر أثرًا قابلًا للملاحظة.
"""


SUPERVISOR_PROJECT_TEMPLATE = """
تكتب هذا التقرير بصفتك مشرفًا تربويًا قاد مبادرة تطويرية داخل الميدان التعليمي.
...
صف فكرة المبادرة، آلية تنفيذها، ودورك في قيادتها ومتابعتها حتى ظهور نتائجها.

وضّح:
- لماذا أطلقت المبادرة
- خطوات التنفيذ
- مشاركة المعلمين
- النتائج الملموسة
...
- كل حقل يقارب 25 كلمة.
- الإجراءات في 4 نقاط.
- تجنب اللغة العامة والإنشائية.
- اذكر نتيجة واقعية ظهرت بعد التنفيذ.
"""


SUPERVISOR_SUPPORT_TEMPLATE = """
تكتب هذا التقرير بصفتك مشرفًا تربويًا مارس دورًا داعمًا في تمكين المعلمين وتحسين المناخ المهني.
...
صف الموقف المهني الذي استدعى تدخلك، وكيف قدمت الدعم، وما التغيير الذي تحقق بعد المتابعة.

ركّز على:
- طبيعة المشكلة أو الاحتياج
- أسلوب التدخل
- استجابة المعلمين
- أثر الدعم
